...
def count_operations(first_string: str, second_string: str) -> int:
    ...
    Dynamic programming approach: table/matrix of dimensions m x n is initialized,
    ...
    for i in range(m + 1):
        for j in range(n + 1):
            if i == 0:  # first_string is empty
                # no. of insertions required = length of second_string
                dp_table[i][j] = j

            elif j == 0:  # second_string empty
                # no. of insertions required = length of first_string
                dp_table[i][j] = i
...
def edit_distance() -> None:
    ...
    """
    first_string = input("Enter first string: ").strip()
    second_string = input("Enter second string: ").strip()
